<template>
  <div class="windows-view">
    <div class="page-header">
      <h1>窗口管理</h1>
      <button @click="showAddDialog = true" class="add-button">+ 添加窗口</button>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="window in windows" :key="window.id">
            ...
            <td>
              <span :class="['status-badge', window.is_online ? 'active' : 'inactive']">
                {{ window.is_online ? '在线' : '离线' }}
              </span>
            </td>
            <td>
              <button @click="editWindow(window)" class="btn-edit">编辑</button>
              <button @click="deleteWindow(window.id)" class="btn-delete">删除</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showAddDialog || showEditDialog" class="dialog-overlay" @click="closeDialogs">
      <div class="dialog-box" @click.stop>
        <h3>{{ showEditDialog ? '编辑窗口' : '添加窗口' }}</h3>
        <form @submit.prevent="submitForm">
          ...
          <div class="form-group">
            <label>窗口编号</label>
            <input v-model.number="formData.window_number" type="number" required />
          </div>
          <div class="form-group" v-if="showEditDialog">
            <label>
              <input type="checkbox" v-model="formData.is_online" />
              在线
            </label>
          </div>
...
export default {
  ...
  setup() {
    ...
    const formData = ref({ name: '', window_number: 1, is_online: true })
    const editingId = ref(null)

    const loadWindows = async () => {
      try {
        loading.value = true
        const response = await axios.get('/admin/windows')
        if (response.success) windows.value = response.data
      } catch (error) {
        alert('加载失败')
      } finally {
        loading.value = false
    ...
    const deleteWindow = async (id) => {
      ...
      try {
        await axios.delete(`/admin/windows/${id}`)
        loadWindows()
      } catch (error) {
        alert('删除失败')
      }
    ...
    const submitForm = async () => {
      try {
        if (showEditDialog.value) {
          await axios.put(`/admin/windows/${editingId.value}`, formData.value)
        } else {
          await axios.post('/admin/windows', formData.value)
        }
        closeDialogs()
        loadWindows()
      } catch (error) {
    ...
    const closeDialogs = () => {
      ...
      formData.value = { name: '', window_number: 1, is_online: true }
      editingId.value = null
    }

    onMounted(loadWindows)

    return { windows, loading, showAddDialog, showEditDialog, formData, editWindow, deleteWindow, submitForm, closeDialogs }
  }
}
</script>

<style scoped>
.windows-view { max-width: 1200px; }
